...
`src-tauri/Entitlements.plist` grants `allow-jit` and `allow-unsigned-executable-memory` for WKWebView's JavaScript engine under the hardened runtime. It has not been re-verified whether current macOS notarized builds still need both; if a release candidate built without them passes notarization and launches correctly, remove them and this note.
