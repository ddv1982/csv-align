...
setup_sha256_url="${CSV_ALIGN_REPOSITORY_SETUP_SHA256_URL:-${setup_url}.sha256}"
setup_sha256_sig_url="${CSV_ALIGN_REPOSITORY_SETUP_SHA256_SIG_URL:-${setup_sha256_url}.asc}"
signing_key_url="${CSV_ALIGN_REPOSITORY_SETUP_SIGNING_KEY_URL:-https://ddv1982.github.io/csv-align/apt/csv-align-archive-keyring.pgp}"
expected_sha256="${CSV_ALIGN_REPOSITORY_SETUP_SHA256:-}"
expected_signing_fingerprint="${CSV_ALIGN_REPOSITORY_SETUP_SIGNING_KEY_FINGERPRINT:-__CSV_ALIGN_APT_SIGNING_KEY_FINGERPRINT__}"
tmp_dir=""
setup_deb=""
setup_sha256_file=""
...
if [ "$expected_signing_fingerprint" = "__CSV_ALIGN_APT_SIGNING_KEY_FINGERPRINT__" ]; then
...
require_expected_keyring_fingerprint() {
  ...
  if ! GNUPGHOME="$gnupg_home" gpg --batch --import-options show-only --with-colons --import "$signing_key_file" >"$keyring_fingerprints_file" 2>/dev/null; then
    echo "Could not inspect downloaded CSV Align setup signing keyring." >&2
    exit 1
  fi
  ...
  case "$keyring_validation" in
    OK)
      ;;
    NO_PRIMARY_KEY)
      echo "Downloaded CSV Align setup signing keyring did not contain a primary key." >&2
      exit 1
      ;;
    MISSING_PINNED_SIGNER)
      echo "Downloaded CSV Align setup signing keyring did not contain the pinned signer fingerprint." >&2
    ...
    UNEXPECTED_PRIMARY_KEY*)
      ...
      echo "Downloaded CSV Align setup signing keyring contained an unexpected primary key." >&2
      echo "Expected signer:    $expected_signing_fingerprint" >&2
      echo "Unexpected primary: $unexpected_primary_key" >&2
      exit 1
      ;;
    MULTIPLE_ACCEPTED_PRIMARY_KEYS)
      echo "Downloaded CSV Align setup signing keyring contained multiple primary keys for the pinned signer." >&2
      echo "Expected signer: $expected_signing_fingerprint" >&2
      exit 1
      ;;
    *)
      echo "Could not validate downloaded CSV Align setup signing keyring." >&2
      exit 1
      ;;
  esac
}
...
verify_sha256_sidecar_signature() {
  ...
  if ! GNUPGHOME="$gnupg_home" gpg --batch --status-fd 3 --verify "$setup_sha256_sig_file" "$setup_sha256_file" 3>"$signature_status_file" >/dev/null 2>&1; then
    echo "CSV Align setup SHA256 signature verification failed." >&2
    exit 1
  fi
  ...
  if [ "$validsig_matches" = "NO_VALIDSIG" ]; then
    echo "CSV Align setup SHA256 signature did not produce a VALIDSIG fingerprint." >&2
    exit 1
  fi

  if [ "$validsig_matches" != "OK" ]; then
    echo "CSV Align setup SHA256 signature was made by an unexpected signer." >&2
...
authenticate_sha256_sidecar() {
  if ! command -v gpg >/dev/null 2>&1; then
    echo "Missing gpg. Install GnuPG or provide CSV_ALIGN_REPOSITORY_SETUP_SHA256 from a trusted source." >&2
    exit 1
  fi
  if [ -z "$expected_signing_fingerprint" ]; then
    echo "Missing pinned CSV Align setup signing key fingerprint." >&2
    echo "Use the release-published installer, or set CSV_ALIGN_REPOSITORY_SETUP_SIGNING_KEY_FINGERPRINT from a trusted source." >&2
    exit 1
  fi

  printf 'Downloading CSV Align repository setup SHA256 sidecar and signature...\n'
  ...
  printf 'Authenticated CSV Align repository setup SHA256 signature with key %s.\n' "$expected_signing_fingerprint"
}

tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/csv-align-repository-setup.XXXXXX")"
setup_deb="$tmp_dir/csv-align-repository-setup_1.0_all.deb"
setup_sha256_file="$tmp_dir/csv-align-repository-setup_1.0_all.deb.sha256"
setup_sha256_sig_file="$tmp_dir/csv-align-repository-setup_1.0_all.deb.sha256.asc"
signing_key_file="$tmp_dir/csv-align-archive-keyring.pgp"
...
if [ -z "$expected_sha256" ]; then
  echo "Could not resolve an authenticated SHA256 for CSV Align repository setup package." >&2
  echo "Use the signed sidecar path, or set CSV_ALIGN_REPOSITORY_SETUP_SHA256 from a trusted source." >&2
  exit 1
fi

actual_sha256="$(calculate_sha256 "$setup_deb")"
if [ "$actual_sha256" != "$expected_sha256" ]; then
  echo "Checksum verification failed for CSV Align repository setup package." >&2
  echo "Expected: $expected_sha256" >&2
  echo "Actual:   $actual_sha256" >&2
  exit 1
fi
printf 'Verified CSV Align repository setup package SHA256.\n'
